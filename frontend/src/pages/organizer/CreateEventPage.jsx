...
export default function CreateEventPage() {
  ...
  const [form, setForm] = useState({
    ...
    is_online: false, capacity: '', is_public: true, status: 'active',
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(data?.detail || Object.values(data || {})[0]?.[0] || 'Failed to create event.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-5">
          ...
          <div>
            ...
            <textarea required value={form.description} onChange={e => set('description', e.target.value)}
              rows={4} className="input resize-none" placeholder="Describe your event..." />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" required min="1" value={form.capacity} onChange={e => set('capacity', e.target.value)} className="input" placeholder="100" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.location} onChange={e => set('location', e.target.value)} className="input" placeholder="City, Venue or 'Online'" />
            </div>
            <div>
      ...
      <style>{`
        ...
        .input:focus { ring: 2px; ring-color: #2563eb; border-color: #2563eb; }
      `}</style>
    </div>
  )
}
